...
int BreadthFirstSearch(cv::Mat& _marker){
  ...
  struct Node {
    ...
    int* m_parent;
    int lv;
    Node(int _x, int _y, int* _m, int* _m_parent, int _lv)
  ...
  for( y = 0; y < size.height; y++ ) {
    for( x = 0; x < size.width; x++ ) {
      ...
      if(x > 0 && m[-1] > 0)
        q1.push( Node(x, y, m, m-1,0) );
      else if(x+1 < size.width && m[1] > 0)
        q1.push( Node(x,y,m, m+1,0) );
      else if(y > 0 && m[-mstep] > 0)
        q1.push( Node(x, y, m, m-mstep,0) );
      else if(y+1 < size.height && m[mstep] > 0)
        q1.push( Node(x, y, m, m+mstep,0) );
    }
    marker += mstep;
  }
  ...
  while(!q1.empty()){
    ...
    *k.m = *k.m_parent;
    if(k.x > 0) {
      int* m_other = k.m-1;
      if(*m_other < 1) // i.e. ==0
        q1.push( Node(k.x-1,k.y,m_other, k.m,k.lv+1) );
    }
    if(k.x < w1 ) {
      int* m_other = k.m+1;
      if(*m_other < 1) // i.e. ==0
    ...
    if(k.y > 0){
      ...
      if(*m_other < 1) // i.e. ==0
        q1.push( Node(k.x,k.y-1,m_other, k.m,k.lv+1) );
    }
    if(k.y < h1){
      int* m_other = k.m+mstep;
      if(*m_other < 1) // i.e. ==0
...
void Segment(const cv::Mat outline_edges, cv::Mat& output) {
  ...
  cv::Mat label_curr;
  for(int lv=0; lv+1 < pyr_edges.size(); lv++){
    ...
    */
    ...
    {
      cv::Mat dst0;
      cv::resize(new_labels, dst0, outline_edges.size(), 0, 0, cv::INTER_NEAREST);
      cv::imshow("new_labels", GetColoredLabel(dst0) );
    }
    */
    label_curr += new_labels;
    label_curr.setTo(0, edge_next);

    //label_curr.setTo(0, label_curr<1);
  }
  /*
