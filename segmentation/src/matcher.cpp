...
std::map<int, std::pair<Mappoint*, double> > FlowMatch(const Camera* camera,
                                                       ...
                                                       bool verbose,
                                                       Frame* curr_frame) {
  const double search_radius = 30.;
  const double search_radius2 = 1.*search_radius;
  ...
  */
  ...
  for(int n0=0; n0 < mappoints0.size(); n0++){
    ...
    std::list< std::pair<int,double> > candidates = curr_frame->SearchRadius(eig_pt1, search_radius2);
    if(candidates.empty())
      continue;
