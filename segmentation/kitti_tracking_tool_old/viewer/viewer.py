...
class Viewer:
    ...
    def add_3D_cars(self,boxes=None,
                     ...
                     show_box_info=False,
                     del_after_show=True,
                     car_model_path=os.path.join(viewer_path,"car.obj"),
    ...
    def show_3D(self):
        ...
        """
        if self.first_show:
            self.vi.show(self.actors+self.actors_without_del, interactive=False, resetcam=False,  camera={'pos': (-15, 0, 5), 'focalPoint': (5, 0, 2), 'viewup': (0, 0, 1)})#
            self.first_show = False
        else:
            self.vi.show(self.actors + self.actors_without_del, interactive=False, resetcam=False)
